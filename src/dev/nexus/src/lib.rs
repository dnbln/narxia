...
use miette::bail;
use narxia_dir_structures::Crate;
use narxia_dir_structures::RustSourceFile;
...
use narxia_dir_structures::display_ws_path;
...
use narxia_dir_structures::resolve_ws_path;
...
use crate::doc_patchup::SessionWrapper;
...
pub enum BuildSysCmd {
    ...
    #[clap(name = "run-doc-extract-daemon")]
    RunDocExtractDaemon,
}
...
async fn render_guide(
    ...
    let code = doc_patchup::patchup_doc(session, rendered).await;

    let output_path = output.map(Cow::Borrowed).or_else(|| {
        let target = guide_dir.canonicalize().ok()?;
        let guides_root = resolve_ws_path!(doc.guides);
        ...
        Some(Cow::Owned(
            resolve_ws_path!(doc.docs.content.docs).join(p.with_extension("mdx")),
        ))
    });

    let Some(output_path) = output_path else {
        item.done(format!(
            "Patchup finished, but guide directory is not in the guides root; skipping write to {}",
            display_ws_path!(doc.docs.content.docs)
        ));
        return Ok(());
    };
...
fn collect_tests_from_source<T: GenericTestDirType>(
    ...
    for entry in paths {
        let entry_path = entry.self_path.clone();
        let file_repo = entry_path.strip_prefix(ws_root()).unwrap();
...
fn collect_tests_from_source_file<T: GenericTestDirType>(
    ...
    let file_contents = file.file.perform_and_store_read().into_diagnostic()?;
...
async fn doc_patchup(
    check: bool,
    cx: &mut NexusContext<'_>,
    item: &mut Item,
    session: &SessionWrapper,
) -> NexusR {
    item.init(None, None);

    for doc in cx.ws.doc.docs.content.docs.iter_mut() {
        let before = doc
            .value_mut()
            .file
            .perform_and_store_read()
            .into_diagnostic()?;
        let code = doc_patchup::patchup_doc(session, (**before).clone()).await;

        match doc_patchup::perform_end(&code, doc.path(), check) {
            Ok(()) => {}
            Err(e) => match e {
