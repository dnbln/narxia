...
```rust ,ignore
pub struct DeferredRead<'a, T, Vfs = crate::FsVfs, const CHECK_ON_READ: bool = {const}>(pub PathBuf, Pin<&'a Vfs>, PhantomData<T>)
```
...
For a cached version see [`DeferredReadOrOwn`](https://docs.rs/dir_structure/0.1.6/dir_structure/deferred_read_or_own/enum.DeferredReadOrOwn.html).
...
```rust ,ignore
pub enum DeferredReadOrOwn<'a, T, Vfs = crate::FsVfs, const CHECK_ON_READ: bool = {const}> {
    Own( /* … */ ),
    Deferred( /* … */ ),
}
```
