...
```rust ,ignore
pub trait DirStructureItem
pub fn write<'a, 'vfs>(&'a self, path: impl AsRef<Path>) -> Result<()>
where
    'vfs: 'a,
...
```rust ,ignore
pub struct DeferredRead<'a, T, Vfs = fs_vfs::FsVfs, const CHECK_ON_READ: bool = {const}>(pub PathBuf, Pin<&'a Vfs>, PhantomData<T>)
```
...
<TooltipTarget>`DirChildren<T, F>::iter#`</TooltipTarget>
...
<TooltipTarget>`DirChildren<T, F>::iter_mut#`</TooltipTarget>
...
```rust ,ignore
impl<'a, const CHECK_ON_READ: bool, T, Vfs> DeferredRead<'a, T, Vfs, CHECK_ON_READ>
...
```rust ,ignore
impl<'a, const CHECK_ON_READ: bool, T, Vfs> DeferredReadOrOwn<'a, T, Vfs, CHECK_ON_READ>
...
<TooltipTarget>`Versioned<T>::edit_eq_check#`</TooltipTarget>
